...
function kursWalut(list) {
    let obj = {};
    let key = list.map(function(item) {
        return item.code
    });

    let value = list.map(function(item) {
        return item.mid
    })
...
//druga wersja 1 zadania

function kursWalut2(lista) {
    let obj = {};
    for(let i = 0, len = lista.length; i < len; i += 1) {
        let row = lista[i];
        obj[row.code] = row.mid;
}
return obj;
}
...
function zad2(){
    let newRow = kursWalut2(data);
    let arr = Object.values(newRow);
    let minValue = Math.min(...arr);
    return minValue
}


 const min = Math.min.apply(null, Object.values(newRow));
